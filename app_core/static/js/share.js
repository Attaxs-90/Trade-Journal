...
/* ---------- Trade teilen (Canvas-Karte) ----------
   ...
   R-Multiple, Ein-/Ausstieg, Dauer) auf einem von 3 Hintergrundmotiven
   (siehe SHARE_BG), per PNG-Download oder Zwischenablage teilbar. Canvas 2D
   statt einer Screenshot-Bibliothek - keine externen Assets/CDN noetig
   (siehe CLAUDE.md), die Hintergrundbilder selbst liegen lokal unter
   /img/share-bg/. */

const SHARE_W = 1080, SHARE_H = 1350, SHARE_PAD = 70;
// Inter/Roboto sind nirgends als Web-Font eingebunden (siehe CLAUDE.md, keine
// externen Font-CDNs) - waren also nie echt aktiv, nur ein Fallback auf
// Systemschriften. "Segoe UI" ist auf Windows immer vorhanden.
const SHARE_FONT = "'Segoe UI', Arial, sans-serif";
...
async function shareDrawLayout(ctx, theme, data) {
  ...
  // Der Betrag ist die wichtigste Zahl der Karte, muss also auf JEDEM Motiv
  // lesbar bleiben - farbiger Text (gruen/rot) verschwindet aber gerade auf
  // den Bulle/Baer-Motiven, die in genau dieser Farbe leuchten. Deshalb eine
  // feste dunkle Plakette dahinter statt sich auf Textfarbe/Schatten allein
  // zu verlassen (siehe Nutzer-Feedback: "Betrag laesst sich schlecht lesen").
  ctx.font = "800 118px " + SHARE_FONT;
  const metricY = badgeY + badgeH + 210;
  const metricTextW = ctx.measureText(data.metricValueText).width;
  const plateW = metricTextW + 96, plateH = 150;
  const plateX = w / 2 - plateW / 2, plateY = metricY - plateH + 40;
  ctx.fillStyle = "rgba(8, 10, 14, 0.62)";
  shareRoundRect(ctx, plateX, plateY, plateW, plateH, 24);
  ctx.fill();
  ctx.strokeStyle = shareHexToRgba(data.accent, 0.55);
  ctx.lineWidth = 2;
  shareRoundRect(ctx, plateX, plateY, plateW, plateH, 24);
  ctx.stroke();
  ...
  ctx.shadowBlur = 10;
  ctx.fillText(data.metricValueText, w / 2, metricY);
  ctx.shadowBlur = 0;
  ...
  const gap = 20;
  const tileW = (w - 2 * SHARE_PAD - (tiles.length - 1) * gap) / tiles.length;
  const tileH = 150;
  const tileY = h - SHARE_PAD - 70 - tileH;
  const tileValueMaxFont = 32, tileValueMinFont = 20;
  tiles.forEach((t, i) => {
    const x = SHARE_PAD + i * (tileW + gap);
    ctx.fillStyle = "rgba(255, 255, 255, 0.07)";
    shareRoundRect(ctx, x, tileY, tileW, tileH, 20);
    ctx.fill();
    ...
    ctx.fillText(t.label, x + tileW / 2, tileY + 48);

    // Wert-Schrift schrittweise verkleinern, falls sie sonst die Kachel
    // sprengen wuerde (z.B. lange Preise) - lieber etwas kleiner als
    // abgeschnitten oder ueberlappend.
    let fontSize = tileValueMaxFont;
    ctx.font = `700 ${fontSize}px ${SHARE_FONT}`;
    const maxTextW = tileW - 24;
    while (fontSize > tileValueMinFont && ctx.measureText(t.value).width > maxTextW) {
      fontSize -= 2;
      ctx.font = `700 ${fontSize}px ${SHARE_FONT}`;
    }
    ctx.fillStyle = "#ffffff";
    ctx.fillText(t.value, x + tileW / 2, tileY + 102);
  });
...
/* Vorschau-Kacheln zeigen das Motiv der tatsaechlichen Trade-Richtung - bei
   einem Short-Trade sonst irrefuehrend immer das gruene Long-Bild (siehe
   Nutzer-Feedback). Deshalb kein fixes Hintergrundbild beim Aufbau der
   Kacheln, sondern erst hier, jedes Mal wenn das Modal fuer einen Trade
   geoeffnet wird. */
function updateShareThemeSwatchPreviews(direction) {
  document.querySelectorAll(".share-theme-swatch").forEach(btn => {
    btn.style.backgroundImage = `url('${SHARE_BG[btn.dataset.theme][direction]}')`;
  });
}
...
function initShareModal() {
  ...
  grid.innerHTML = SHARE_THEMES.map(t => `
    <button type="button" class="share-theme-swatch" data-theme="${t.id}">
...
export function openShareModal(trade) {
  ...
  shareState = { trade, metric: "net", themeId: shareState.themeId || "clean" };
  ...
  updateShareThemeSwatchPreviews(trade.direction);
  const grid = document.getElementById("share-theme-grid");
