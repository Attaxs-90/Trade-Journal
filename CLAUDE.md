...
- **Löschen ist zweigleisig:** `archived = 1` ist der Normalfall (Trades und Auswertung bleiben), `DELETE` entkoppelt die Trades wie `delete_account()`. Eine Gruppe zu löschen löst sie nur auf — ihre Regeln samt Bewertungen bleiben und rutschen auf „ohne Gruppe".
